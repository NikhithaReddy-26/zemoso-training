...
function generateBill() {
  ...
  for (var i = 1; i < table.rows.length; i++) {
    ...
    billWindow.document.write(
      `<tr><td>${itemName}</td><td>${itemQuantity}</td><td>${itemPrice}</td></tr>`
    );
  }
  billWindow.document.write("</table>");
  billWindow.document.write(`<p>Total Items: ${totalItems}</p>`);
  billWindow.document.write(`<p>Total Cost: ${totalCost}</p>`);

  billWindow.addEventListener("beforeunload", function () {
